...
给本项目中的前端页面添加一个按钮，用来一键获取东方财富的2=行业板块，3=概念板块，以及同花顺。当然必须传入 cookie 值。上面3个请求，一步步执行。执行完成后刷新页面。

### history

_fetch_page_raw_httpx 中 以及其他地方移除 ut 参数，也不需要截获动态 ut 了，只要模拟打开获取 cookie 即可

### history

优化这个项目中的数据库表的 generate 与 migrate 操作，就像 nodejs 中的 drizzle orm 一样。这样更加规范，数据库也更好迁移。

### history

再添加一个 post 接口，因为东方财富的获取信息接口大概率会报错，所以我希望添加一个接口，我会给你我拿到的jsonp数据，你直接写入数据库，但我只能分几次给你，你自己进行按日期新增或更新
我给你如下（个数不确定）
jQuery371023459551565698888_1772588943343({"rc":0,"rt":6,"svr":177617647,"lt":1,"full":1,"dlmkts":"","data":{"total":468,"diff":[{"f3":384,"f6":48176182420.0,"f8":250,"f14":"特高压","f20":2047730976000},{"f3":321,"f6":70166395606.0,"f8":272,"f14":"智能电网","f20":3071526592000},{"f3":277,"f6":6983970049.0,"f8":196,"f14":"船舶制造","f20":599323152000},{"f3":269,"f6":20792140429.0,"f8":217,"f14":"虚拟电厂","f20":961982560000},{"f3":262,"f6":3192028567.0,"f8":323,"f14":"地热能","f20":113059175000},{"f3":259,"f6":20183492831.0,"f8":248,"f14":"高压快充","f20":831470160000},{"f3":258,"f6":21872514948.0,"f8":145,"f14":"抽水蓄能","f20":1224184832000},{"f3":248,"f6":66168349895.0,"f8":204,"f14":"存储芯片","f20":4451198976000},{"f3":233,"f6":22651705260.0,"f8":238,"f14":"Chiplet概念","f20":1212335152000},{"f3":225,"f6":12212904219.0,"f8":262,"f14":"被动元件概念","f20":618110240000}]}});

### history

给 / 根目录写一个静态页，用来展示东方财富与同花顺的板块数据。
东方财富与同花顺的切换使用 tab 方式
使用表格展示，可以搜索。

### history

添加一个接口用来专门触发 fetch_eastmoney_sectors 来获取板块数据
这是一个 post 请求，我可以不传参数，也可以传 ut cookie 这些校验参数。

### history

/market/sectors
/market/ths_sectors
将这两个接口先注释调改为定时爬取数据，每15分钟获取数据，并存进数据库，并添加上更新时间。
当天第一个执行为添加操作，后续都为更新操作
这两个数据可能返回为空。所以为空就跳过，不更新
这两个接口返回板块数量不一致，所以用两张表存数据。
数据库连接使用 .env 环境变量来控制 DB_URL = "postgresql://"
并在第一次使用时创建好 schema 与数据表结构



/market/sectors
/market/ths_sectors
将这两个接口改为直接读取当日的最新数据库数据
/market/df_sectors
/market/ths_sectors
再添加一个接口，用来返回两个接口的板块字段比如
{
  "东方财富": [
    "板块名称1",
    "板块名称2"
...
东方财富jsonp 参数ut 与 cookie 是联动的，必须动态获取必须先直接访问 https://quote.eastmoney.com/center/gridlist.html#industry_board，
时会获取到 https://push2.eastmoney.com/api/qt/clist/get 接口中自动生成的 ut 字段以及 cookie值，然后用将这些值替换掉原来的写死的 cookie以及ut参数

### history

我发现 同花顺数据 的数据接口如果不带 header 会直接 401，必须带上 hexin-v 字段，可这个不知道怎么获取。但直接使用浏览器访问就直接访问成功，能不能帮我直接改为模拟浏览器访问或者你有更好的方案。

### history

使用 AkShare 完成两个接口数据的获取以及测试
1. 所有板块的成交额占比
公式：某板块当日总成交额 ÷ 沪深两市当日总成交额 × 100%
2. 所有板块的换手率
公式：板块当日成交股数 ÷ 板块流通总股数 × 100%

### history

将这个项目添加上 AKShare 的公墓基金数据接口
参考文档 https://akshare.akfamily.xyz/data/fund/fund_public.html

主要是通过 fundCode 查看详细信息
基金基本信息-雪球
...
帮我给当前的项目添加一个接口，/charts/rsi/{fundCode}
